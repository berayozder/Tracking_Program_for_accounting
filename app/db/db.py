...
try:
    ...
    from .crypto_utils import encrypt_str, decrypt_str  # type: ignore
...
def _get_rate_generic(date_str: str, from_ccy: str, to_ccy: str) -> Optional[float]:
    ...
    try:
        ...
        import fx_rates  # local import to avoid circular at module load
    ...
    if IMPORTS_CSV.parent.joinpath('expenses.csv').exists():
        ...
        with EXP_CSV.open(newline='') as f:
            ...
            for r in reader:
                ...
                cur.execute('''INSERT INTO expenses (date, amount, is_import_related, import_id, category, notes)
                            VALUES (?,?,?,?,?,?)''', (
                    r.get('Date'), float_or_none(r.get('Amount')), is_imp, imp_id, r.get('Category'), r.get('Notes')
                ))
                # Create link row for migration if import_id provided
                try:
                    new_expense_id = cur.lastrowid
                    if imp_id:
                        cur.execute('INSERT OR IGNORE INTO expense_import_links (expense_id, import_id) VALUES (?,?)', (new_expense_id, imp_id))
                except Exception:
                    pass
        conn.commit()
...
def add_expense(date, amount, is_import_related=False, import_id=None, category=None, notes=None, document_path=None, import_ids=None, currency: str | None = None):
    ...
    - import_id: single legacy link; used if import_ids not provided.
    """
    conn = get_conn()
    cur = conn.cursor()
...
def edit_expense(expense_id, date, amount, is_import_related=False, import_id=None, category=None, notes=None, document_path=None, import_ids=None, currency: str | None = None):
    conn = get_conn()
    cur = conn.cursor()
