...
def _bootstrap_admin(root) -> bool:
    ...
    # Increased default size so all fields & action buttons are visible on first open
    # (previous 380x240 could clip buttons on some DPI/theme settings)
    dlg.geometry('500x340')
    try:
        # Provide a generous minimum size while still allowing manual resize
        dlg.minsize(480, 320)
    except Exception:
        pass
    dlg.transient(root)
