...
DESIRED_COLS = ['Date', 'Category', 'Subcategory', 'Quantity', 'UnitPrice', 'SellingPrice', 'Platform', 'ProductID', 'CustomerID', 'DocumentPath']
...
def open_view_sales_window(root):
        ...
        for idx, r in enumerate(all_rows):
            if row_matches_year(r, yy) and row_matches_search(r, q) and row_matches_returned(r, return_var.get(), returned_ids):
                ...
                try:
                    ...
                    pass
        totals_var.set(f"Items: {shown}    Total Selling: {total_sell:.2f}")
        try:
    ...
    def do_attach():
        ...
        if idx is None:
            messagebox.showwarning('Select', 'Select a row first')
            return
        # Use native file picker for selecting a document
        path = filedialog.askopenfilename(parent=win, title='Attach document')
        if not path:
            return
        from pathlib import Path as _P
        try:
        ...
        rows = read_sales()
        if 0 <= idx < len(rows):
            rows[idx]['DocumentPath'] = attach.strip()
            write_sales(rows)
            refresh()
    ...
    def do_open_doc():
        ...
        rows = read_sales()
        if 0 <= idx < len(rows):
            docp = rows[idx].get('DocumentPath', '')
            _open_default(docp)
    ...
    ttk.Button(secondary_frame, text='📎 Attach Document', 
              command=do_attach).pack(side=tk.LEFT, padx=4)
    ttk.Button(secondary_frame, text='📄 Open Document', 
              command=do_open_doc).pack(side=tk.LEFT, padx=4)
