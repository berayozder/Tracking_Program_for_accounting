"""Backfill script: export DB tables to CSVs in data/.

Usage:
    python -m py.db.export_csv             # Export missing CSVs only
    python -m py.db.export_csv --overwrite # Force regenerate CSVs

Exports:
    - data/imports.csv
    - data/expenses.csv
Creates a timestamped .bak copy before overwriting an existing non-empty file.
"""
...
def export_imports(target: Path, overwrite: bool):
    # Fetch all imports directly (no artificial limit)
    conn = db.get_conn()
    cur = conn.cursor()
    ...
    with target.open('w', newline='') as f:
        ...
        w.writerow(headers)
        for r in rows:
            # notes are encrypted in DB; decrypt
            try:
                notes = db.decrypt_str(r['notes'])  # type: ignore[attr-defined]
            except Exception:
                notes = r['notes']
            w.writerow([
                r['id'], r['date'], r['ordered_price'], r['quantity'], r['supplier'],
                r['category'], r['subcategory'], r['currency'], notes
            ])
...
def export_expenses(target: Path, overwrite: bool):
    ...
    with target.open('w', newline='') as f:
        ...
        for r in rows:
            ...
            w.writerow([
                r['id'], r['date'], r['amount'], r['is_import_related'], r['import_id'],
                ';'.join(str(x) for x in linked), r['category'], notes, r['document_path'], r['currency']
            ])
